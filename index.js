...
const fs      = requir/**
 * ═══════════════════════════════════════════════════════════════════
 *  SOLANA COPY BOT — Single Wallet Mirror
 *  Mirrors every trade taken by one target wallet, exactly.
...
const CFG = {
  ...
  // ── The one wallet to copy ─────────────────────────────────────────────
  TARGET_WALLET:    process.env.TARGET_WALLET || "AMRsSeU5JpqwQWJGNLMpZzRCZSFEwYQYbMnms3dD4311",

  // ── Sizing — pure % of your balance, no hard SOL cap ──────────────────
  TRADE_PCT:        parseFloat(process.env.TRADE_PCT    || "0.05"),  // 5% per trade
  MIN_TRADE_SOL:    parseFloat(process.env.MIN_TRADE    || "0.01"),  // tiny floor for fees
  RESERVE_SOL:      parseFloat(process.env.RESERVE_SOL  || "0.05"),  // keep for fees

  // ── Backstop exits (in case target doesn't sell in time) ───────────────
  HARD_STOP_PCT:    parseFloat(process.env.HARD_STOP    || "0.10"),  // -10% hard stop
  MAX_HOLD_MS:      parseInt(  process.env.MAX_HOLD     || "1800000"),// 30 min max hold

  // ── Circuit breakers ───────────────────────────────────────────────────
  DAILY_LOSS_PCT:   parseFloat(process.env.DAILY_LOSS   || "0.30"),  // halt at -30% day
  MAX_FAILED_SWAPS: parseInt(  process.env.MAX_FAIL     || "5"),
  FAILED_SWAP_WINDOW_MS: 3600000,

  // ── PumpPortal (handles bonding curve + Raydium, pool:"auto") ──────────
  ...
  POLL_MS:          parseInt(  process.env.POLL_MS      || "3000"),  // check target every 3s
  TX_MAX_AGE_SEC:   90,   // ignore txs older than 90s — avoids replaying old trades
...
// ─────────────────────────────────────────────────────────────────────────────
let state = {
  positions:       {},   // tokenMint → { buyPrice, rawAmount, timestamp, amountSol }
...
function loadState() {
  try {
    if (fs.existsSync(CFG.STATE_FILE)) {
      ...
      state.lastSeenSig   = s.lastSeenSig   || null
...
async function getSolBalance(pubkey) {
  try { return (await connection.getBalance(pubkey)) / LAMPORTS_PER_SOL }
  catch (e) { log("ERROR", `Balance failed: ${e.message}`); return null }
}

async function getTokenBalance(walletPubkey, tokenMint) {
  try {
    const accs = await connection.getParsedTokenAccountsByOwner(
      walletPubkey, { mint: new PublicKey(tokenMint) }
    )
    if (!accs.value.length) return "0"
    return accs.value[0].account.data.parsed.info.tokenAmount.amount
  } catch (e) { log("ERROR", `Token balance failed: ${e.message}`); return "0" }
}

// ─────────────────────────────────────────────────────────────────────────────
//  SIZING — pure percentage, no hard SOL cap
// ─────────────────────────────────────────────────────────────────────────────
...
async function pumpPortalSwap(wallet, tokenMint, action, amount, label = "") {
  ...
  if (!res.ok) throw new Error(`PumpPortal HTTP ${res.status}: ${(await res.text().catch(()=>"")).slice(0,200)}`)
  ...
  tx.sign([wallet])
  const sig = await connection.sendTransaction(tx, { skipPreflight: false, preflightCommitment: "confirmed", maxRetries: 3 })
...
async function jupiterSwap(wallet, inputMint, outputMint, amountLamports, label = "") {
  const headers  = { "Content-Type": "application/json", ...(CFG.JUP_API_KEY ? { "x-api-key": CFG.JUP_API_KEY } : {}) }
  const params   = new URLSearchParams({ inputMint, outputMint, amount: amountLamports.toString(), taker: wallet.publicKey.toString() })
  ...
  if (result?.status !== "Success") throw new Error(`Jup not successful: ${JSON.stringify(result).slice(0,200)}`)
...
async function doBuy(wallet, tokenMint, amountSol, label) {
  try {
    ...
    return await jupiterSwap(wallet, SOL_MINT, tokenMint, Math.floor(amountSol * LAMPORTS_PER_SOL), label + "_JUP")
  }
}
...
//  COPY BUY
// ─────────────────────────────────────────────────────────────────────────────
async function copyBuy(wallet, tokenMint) {
  ...
  log("BUY", `Copying buy | ${tokenMint.slice(0,8)}... | ${amountSol.toFixed(4)}SOL | price:${buyPrice || "unknown"}`)
  ...
  try {
    ...
    logCsv({ type: "BUY", token: tokenMint, amountSol, price: buyPrice || 0 })
  } catch (e) {
...
async function copySell(wallet, tokenMint, reason = "TARGET_SOLD") {
  ...
  if (!pos) { log("SKIP", `No position for ${tokenMint.slice(0,8)}...`); return }
  ...
  try {
    ...
    logCsv({ type: "SELL", token: tokenMint, amountSol: pos.amountSol, price: sellPrice || 0, pct, result: isWin ? "WIN" : "LOSS", reason })
...
// ─────────────────────────────────────────────────────────────────────────────
//  BACKSTOP MONITOR — fires if target doesn't sell and we hit our limits
// ─────────────────────────────────────────────────────────────────────────────
...
//  Checks every POLL_MS for new txs from the target.
//  Detects buys (token balance up) and sells (token balance down/zero).
// ─────────────────────────────────────────────────────────────────────────────
async function pollTarget(wallet) {
  const targetPubkey = new PublicKey(CFG.TARGET_WALLET)
  let sigs
  try {
    sigs = await connection.getSignaturesForAddress(targetPubkey, { limit: 10 })
  } catch (e) { log("WARN", `Sig fetch failed: ${e.message}`); return }
  if (!sigs?.length) return

  const nowSec  = Math.floor(Date.now() / 1000)
  const newSigs = []
  for (const s of sigs) {
    if (s.signature === state.lastSeenSig) break
    if (s.blockTime && (nowSec - s.blockTime) > CFG.TX_MAX_AGE_SEC) continue
    if (s.err) continue
    newSigs.push(s)
  }
  if (sigs[0]) state.lastSeenSig = sigs[0].signature
  if (!newSigs.length) return

  log("COPY", `${newSigs.length} new tx(s) from target — parsing...`)
  ...
  for (const sigInfo of newSigs.reverse()) {
    try {
      ...
      for (const postBal of post) {
        ...
        if (postAmt > preAmt) {
          log("COPY", `Target BOUGHT ${postBal.mint.slice(0,8)}... (${preAmt} → ${postAmt})`)
          await copyBuy(wallet, postBal.mint)
        }
      }
      ...
      for (const preBal of pre) {
        ...
        if (!state.positions[preBal.mint]) continue  // we don't hold it, nothing to do
        ...
        if (preAmt > 0 && postAmt < preAmt) {
          ...
          log("COPY", `Target SOLD (${type}) ${preBal.mint.slice(0,8)}... (${preAmt} → ${postAmt})`)
          // Always do full exit on our side to keep it simple
          await copySell(wallet, preBal.mint, `TARGET_SOLD_${type}`)
        }
      }
    } catch (e) { log("WARN", `Tx parse error: ${e.message}`) }
  }
...
function logCsv(t) {
  ...
  } catch (e) { log("ERROR", `CSV log failed: ${e.message}`) }
}
...
async function printSummary(walletPubkey) {
  ...
  console.log(`\n[${ts()}] 📊 ═══════════════ SUMMARY ═══════════════`)
  console.log(`  Copying:        ${CFG.TARGET_WALLET}`)
...
async function runBot() {
  log("START", "═══════════════════════════════════════════")
  log("START", "  Solana Copy Bot — Single Wallet Mirror   ")
  log("START", "═══════════════════════════════════════════")

  const wallet = loadWallet()
  log("START", `Your wallet:  ${wallet.publicKey.toString()}`)
  log("START", `Copying:      ${CFG.TARGET_WALLET}`)
  ...
  // Initialize cursor — don't replay trades from before bot started
  if (!state.lastSeenSig) {
    try {
      const sigs = await connection.getSignaturesForAddress(new PublicKey(CFG.TARGET_WALLET), { limit: 1 })
      if (sigs?.[0]) {
        state.lastSeenSig = sigs[0].signature
        log("START", `Cursor set to latest sig — won't replay old trades`)
      }
    } catch (e) { log("WARN", `Cursor init failed: ${e.message}`) }
  }
